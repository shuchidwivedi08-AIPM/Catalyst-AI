...
def _clear_workflow_session() -> None:
    """Prevent in-memory workflow data from crossing project boundaries."""
    ...
    with st.expander("Create new project", expanded=False):
        with st.form("create_project_form", clear_on_submit=True):
            ...
            description = st.text_area(
                "Description",
                max_chars=1000,
                help="Briefly describe the product, initiative, or business problem.",
            )
...
def render_project_dashboard(user) -> None:
    """Show all projects the authenticated user can access."""
...
def get_selected_project(user) -> Project | None:
    """Resolve selected project and remove stale or unauthorized selections."""
    initialize_project_session()
...
def render_project_sidebar(project: Project) -> None:
    """Keep the active project visible throughout its workflow."""
    with st.sidebar:
